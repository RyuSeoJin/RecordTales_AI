...
project-plan/
├── approved/                       ← 승인된 시스템 스펙 (리팩터 이후 본격 작성)
│   └── guild-info.md               246줄. 첫 시도 — 참고용으로 유지 중
└── draft/                          ← 아이디어 단계 시스템
    ├── tutorial-flow.md            95줄.  최초 실행 튜토리얼
    └── experience-system.md        111줄. 경험치·레벨링

(향후 작성 예정)
project-roadmap/
├── project-data.md                 데이터 스키마 — 프로젝트 진행 후 작성
└── project-wiki/                   승격된 문서가 이동하는 곳 (현재 비어있음)

project-plan/draft/
├── guild-member-hiring.md          미작성
├── squad-formation.md              미작성
└── environment-settings.md         미작성

project-plan/approved/ (미작성 10종)
...
3. **`project-plan/draft/` 2종 확인**
4. **`project-plan/approved/guild-info.md` 참고** (재작성 필요 시 스타일 기준)
